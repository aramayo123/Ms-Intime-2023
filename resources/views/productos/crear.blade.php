...
<div class="contenedorPerfil">
    ...
    <form action="{{ route('productos.store') }}" method="post" enctype="multipart/form-data" class="formPerfil">
        @csrf
        <!-- TITLE !-->
        <label for="title">Titulo del producto</label>
        <x-text-input class="form-control" type="text" name="title" :value="old('title')" autofocus
            autocomplete="title" id="title" />
        <!-- ERRRORS TITLE !-->
        @if ($errors->get('title'))
        ...
        <x-text-input type="text" class="form-control" name="description" :value="old('description')" id="description" />
        <!-- ERRRORS DESCRIPTION !-->
        ...
        <div class="opcionesColor">
            @php
                ...
                for ($value = 0; $value < count($colores_arr); $value++) {
                    $checked = old('color_' . $colores_arr[$value]) ? 'checked' : '';
                    echo '
        ...
        <div class="opcionesTalle">
            @php
                ...
                for ($value = 0; $value < count($talles_arr); $value++) {
                    $checked = old('talle_' . $talles_arr[$value]) ? 'checked' : '';
                    echo '
        ...
        @if ($errors->get('price_send'))
            ...
            @endforeach
        @endif
        <button class="enviarPerfil boton">Crear producto</button>
    </form>
</div>
</main>
